$(document).ready(function() {
    // if (window.location.hostname == tmbq) {
        ...
        var demoanhurl = $(".demoanhurl").hide();
        var b1 = $(".b1");
        var b2 = $(".b2").hide();
        var b02 = $(".b02").hide();
        var b3 = $(".b3").hide();
        ...
        var fonts = ["Roboto", "VT323", "Pacifico", "Amatic SC", "Lobster"];
        ...
        canvas.on({
            'object:selected': function() {
                ...
                $("#blendmode").val(obsel.globalCompositeOperation);
                b2.hide();
                b3.show();
                $("#opacity").val(obsel.opacity);
            },
        ...
        function nuttim() {
            demoanhurl.show();
            var inputurl = $("#tad-nhapurl input").val();
            if (inputurl == "") {
                alert("Url ảnh không được để trống");
                $("#tad-nhapurl input").focus();
                return false
            }
            window.location.hash = inputurl;
            $("#tad-urlimg").html("<img  src='" + inputurl + "' class='img-thumbnail w-100' id='imgurl' alt='Tạo ảnh đẹp'/>");
        ...
        $("#tad-nhapurl button").click(function() {
            nuttim();
        });
        $("#chenchutad").click(function() {
            var max = 500;
            canvas.setDimensions({
                width: max,
                height: max * tyle
            });
            canvas.setBackgroundImage(urlimg, canvas.renderAll.bind(canvas), {
                width: max,
                height: max * tyle,
            });
            b2.show();
        ...
        var urlimg = decodeURIComponent(window.location.hash).substring(1);
        if (urlimg != "") {
            $("#tad-nhapurl input").val(urlimg);
            $("#tad-urlimg").html("<img src='" + urlimg + "' class='img-thumbnail w-100' id='imgurl' alt='Tạo ảnh đẹp'/>");
            nuttim();
        }
        ...
        $("#okText").click(function() {
            ...
            canvas.getActiveObject().set("fontFamily", fontht);
            canvas.getActiveObject().set("fill", mauchuht);
            canvas.renderAll();
            canvas.deactivateAll();
        });
        canvas.on({
            'text:selection:changed': function() {
                addText.hide();
                okText.show();
                textsel = canvas.getActiveObject();
                $('#chenchu').modal('show');
                $("#inputtext").val(textsel.text)
            }
        });
        var fontht = "Arial";
        var mauchuht = "#ffffff"
        ...
        document.getElementById('font-family').onchange = function() {
            if (this.value !== 'Arial') {
                $("#inputtext").attr("style", "font-family:" + this.value);
        ...
        $("#mauchu").on("change", function() {
            mauchuht = $(this).val();
        });
        $("#tad-themanh").on("input", function(e) {
            b4.show();
            var file = e.target.files[0];
            var reader = new FileReader();
            reader.onload = function(f) {
        ...
        blendmode.forEach(function(blendmode) {
            var option = document.createElement('option');
            option.innerHTML = blendmode.name;
            option.value = blendmode.mode;
            select.appendChild(option);
        });
        document.getElementById('blendmode').onchange = function() {
            canvas.getActiveObject().set("globalCompositeOperation", this.value);
            canvas.renderAll();
        };
        $("#opacity").on("input", function() {
            canvas.getActiveObject().set("opacity", $(this).val());
            canvas.renderAll();
        });
        $("#showopacity").click(function() {
            b13.hide();
            b23.show();
        });
        $("#qlopacity").click(function() {
            b13.show();
            b23.hide();
        });
        ...
        $('#dongtad .co').click(function() {
            canvas.clear();
            dongbang();
        });
        $('#dongtad .khong').click(function() {
            dongbang();
        });
        ...
        var tenanh = "Tạo ảnh đẹp";
        var dinhdang = "jpg";
        var kichco = 2;
        $(".dinhdang input").click(function() {
            dinhdang = $("input:checked").val()
        });
        $(".kichco").change(function() {
            kichco = $(this).val()
        });
        $(".tenanh").on("input", function() {
            tenanh = $(".tenanh").val()
        });
        $("#tad-luuanh").click(function() {
            canvas.deactivateAll();
            var a = canvas.width;
            var b = canvas.height;
            ...
            canvas.renderAll();
            $("#canvas").get(0).toBlob(function(c) {
                saveAs(c, tenanh + ".png")
            });
            canvas.setZoom(1);
            canvas.setDimensions({
