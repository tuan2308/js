    ...
    if (window.location.hostname == tmbq) {
        ...
        $("#tad-taoanh").click(function() {
            ...
            html2canvas(document.querySelector(".tad-main"), { useCORS: !0, scrollX: 0, scrollY: 0, width: rongh2, height: daih2, scale: 1, x: 0, y: 0 }).then(canvas => { canvas.toBlob(function(blob) { var bloburl = URL.createObjectURL(blob);
                    $("#img-out").html("<label class='mt-2 tad-ketqua'> Kết quả: <button type='button' class='badge btn btn-primary tad-xemtruoc' data-toggle='modal' data-target='#exampleModal'> Xem trước </button></label><img src='" + bloburl + "' alt='Tạo ảnh đẹp' class='img-thumbnail'></img><a href='" + bloburl + "' class='btn btn-block btn-primary mt-2' download='taoanhdep_" + tenanhxuat + ".jpg'><i class='fas fa-cloud-download-alt'></i> Tải ảnh về</a>");
                    $(".tad-xemtruoc").click(function() { $(".ip-style").html("<style>.tad-screen, .tad-bg:before { background: url(" + bloburl + ") center; }</style>");
                        $(".btn-tav").attr("download", "taoanhdep_" + tenanhxuat + ".jpg").attr("href", bloburl); }, 'image/jpeg'); });
