	...
	if (window.location.hostname == tmbq) {
		...
		function i() {
			...
			a.onload = function() {
				...
				f.src = bgimg;
				f.onload = function() {
					c = document.querySelector("canvas"), ctx = c.getContext("2d"), corners = d, step = tadStep;
					var D, h, g, A, q, E, B, l, s = e.width - 1,
						n = e.height - 1;
					ctx.clearRect(0, 0, c.width, c.height);
					ctx.save();
					for (y = 0; y < n; y += step) {
						for (x = 0; x < s; x += step) {
							q = p(corners[0], corners[3], y / n);
							E = p(corners[1], corners[2], y / n);
							B = p(corners[0], corners[3], (y + step) / n);
							l = p(corners[1], corners[2], (y + step) / n);
							D = p(q, E, x / s);
							h = p(q, E, (x + step) / s);
							g = p(B, l, (x + step) / s);
							A = p(B, l, x / s);
