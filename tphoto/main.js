canvas=new fabric.Canvas('canvas');canvas.renderAll.bind(canvas)
var b1=$(".b1");var b2=$(".b2").hide();var b02=$(".b02").hide();var b3=$(".b3").hide();var b13=$(".b13");var b23=$(".b23").hide();var b4=$(".b4").hide();canvas.selection=false;var fonts=["Roboto","VT323","Pacifico","Amatic SC","Lobster","Pattaya"];fabric.Object.prototype.customiseCornerIcons({settings:{borderColor:'white',cornerSize:10,cornerShape:'circle',cornerBackgroundColor:'white',cornerPadding:20}});var blendmode=[{mode:"source-over",name:"Hoà trộn"},{mode:"screen",name:"Bộ lọc"},{mode:"multiply",name:"Nhiều lớp"},{mode:"overlay",name:"Xếp chồng"},]
...
else{if(img.width>img.height){if(img.width>max){let scale=max/img.width;img.set({scaleX:scale,scaleY:scale});canvas.setDimensions({width:max,height:img.height*scale});}}
...
canvas.setBackgroundImage(img);canvas.renderAll();});};reader.readAsDataURL(file);$("#tad-upnen").val("")});$("#tad-chenchu").click(function(){$("#inputtext").val("");addText.show();okText.hide();});var addText=$("#addText");var okText=$("#okText").hide();$("#addText").click(function(){var tw=canvas.width/2;var nd=$("#inputtext").val();var textbox=new fabric.Textbox(nd,{width:tw,left:canvas.width/2-tw/2,top:canvas.height/2,fontSize:tw/5,fontFamily:fontht,fill:mauchuht,shadow:new fabric.Shadow({color:"rgba(0, 0, 0, 0.2)",blur:2,offsetX:2,offsetY:2})});canvas.add(textbox).setActiveObject(textbox);b4.show();});$("#okText").click(function(){canvas.getActiveObject().set("text",$("#inputtext").val());canvas.getActiveObject().set("fontFamily",fontht);canvas.getActiveObject().set("fill",mauchuht);canvas.renderAll();});canvas.on({'text:selection:changed':function(){addText.hide();okText.show();textsel=canvas.getActiveObject();$('#chenchu').modal('show');$("#inputtext").val(textsel.text)
...
$("#mauchu").on("change",function(){mauchuht=$(this).val();});$("#tad-themanh").on("input",function(e){b4.show();var file=e.target.files[0];var reader=new FileReader();reader.onload=function(f){var data=f.target.result;fabric.Image.fromURL(data,function(img){var max=500;if(img.width<max||img.height<max){img.set({left:canvas.width/2-img.width*0.5/2,top:canvas.height/2-img.height*0.5/2}).scale(0.5);}
else{if(img.width>img.height){if(img.width>max){let scale=(max/img.height)*0.5;img.set({left:canvas.width/2-img.width*scale/2,top:canvas.height/2-img.height*scale/2}).scale(scale);}}
...
$("#backb1").click(function(){$('#dongtad').modal('show');});$('#dongtad .co').click(function(){canvas.clear();dongbang();});$('#dongtad .khong').click(function(){dongbang();});$("#backb2").click(function(){b2.show();b3.hide();});$("#tad-luuanh").click(function(){canvas.deactivateAll();canvas.renderAll();$("#canvas").get(0).toBlob(function(blob){saveAs(blob,"taoanhdep.png");});});