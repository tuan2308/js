$(document).ready(function(){if(window.location.hostname==tmbq){canvas=new fabric.Canvas('canvas');canvas.renderAll.bind(canvas)
var b1=$(".b1");var b2=$(".b2").hide();var b02=$(".b02").hide();var b3=$(".b3").hide();var b13=$(".b13");var b23=$(".b23").hide();var b4=$(".b4").hide();canvas.selection=false;var fonts=["Roboto","VT323","Pacifico","Amatic SC","Lobster"];fabric.Object.prototype.customiseCornerIcons({settings:{borderColor:'white',cornerSize:10,cornerShape:'circle',cornerBackgroundColor:'white',cornerPadding:20}});var blendmode=[{mode:"source-over",name:"Hoà trộn"},{mode:"screen",name:"Bộ lọc"},{mode:"multiply",name:"Nhiều lớp"},{mode:"overlay",name:"Xếp chồng"},]
...
canvas.setBackgroundImage(img);canvas.renderAll();});};reader.readAsDataURL(file);$("#tad-upnen").val("")});$("#tad-chenchu").click(function(){$("#inputtext").val("");addText.show();okText.hide();});var addText=$("#addText");var okText=$("#okText").hide();$("#addText").click(function(){var tw=canvas.width/2;var nd=$("#inputtext").val();if(nd.length==0){alert("Vui lòng nhập chữ");$("#inputtext").focus();return false}
var textbox=new fabric.Textbox(nd,{width:tw,left:canvas.width/2-tw/2,top:canvas.height/2,fontSize:tw/5,textAlign:'center',fontFamily:fontht,fill:mauchuht,shadow:new fabric.Shadow({color:"rgba(0, 0, 0, 0.2)",blur:2,offsetX:2,offsetY:2})});canvas.add(textbox).setActiveObject(textbox);b4.show();});$("#okText").click(function(){if($("#inputtext").val().length==0){alert("Vui lòng nhập chữ");$("#inputtext").focus();return false}
canvas.getActiveObject().set("text",$("#inputtext").val());canvas.getActiveObject().set("fontFamily",fontht);canvas.getActiveObject().set("fill",mauchuht);canvas.renderAll();});canvas.on({'text:selection:changed':function(){addText.hide();okText.show();textsel=canvas.getActiveObject();$('#chenchu').modal('show');$("#inputtext").val(textsel.text)
console.log(textsel)}});var fontht="Arial";var mauchuht="#ffffff"
...
$("#backb1").click(function(){$('#dongtad').modal('show');});$('#dongtad .co').click(function(){canvas.clear();dongbang();});$('#dongtad .khong').click(function(){dongbang();});$("#backb2").click(function(){b2.show();b3.hide();});var tenanh="Tạo ảnh đẹp";var dinhdang="jpg";var kichco=2;$(".dinhdang input").click(function(){dinhdang=$("input:checked").val()});$(".kichco").change(function(){kichco=$(this).val()});$(".tenanh").on("input",function(){tenanh=$(".tenanh").val()});$("#tad-luuanh").click(function(){canvas.deactivateAll();var a=canvas.width;var b=canvas.height;canvas.setZoom(kichco);canvas.setDimensions({width:a*kichco,height:b*kichco});canvas.renderAll();$("#canvas").get(0).toBlob(function(c){saveAs(c,tenanh+"."+dinhdang)});canvas.setZoom(1);canvas.setDimensions({width:a,height:b})});}});