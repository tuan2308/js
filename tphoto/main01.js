...
canvas.setBackgroundImage(img);canvas.renderAll();});};reader.readAsDataURL(file);$("#tad-upnen").val("")});function GetURLParameter(sParam){var sPageURL=window.location.search.substring(1);var sURLVariables=sPageURL.split('&');for(var i=0;i<sURLVariables.length;i++){var sParameterName=sURLVariables[i].split('=');if(sParameterName[0]==sParam){return sParameterName[1];}}}
var urlimg0=GetURLParameter('img');var urlimg=decodeURIComponent(urlimg0);if(urlimg0!=undefined){demoanhurl.show();$("#tad-nhapurl input").val(urlimg);$("#tad-urlimg").html("<img src='"+urlimg+"' class='img-thumbnail w-100' id='imgurl' alt='Tạo ảnh đẹp'/>");}
else{demoanhurl.hide()}
$("#tad-nhapurl button").click(function(){demoanhurl.show();urlimg=$("#tad-nhapurl input").val();if(urlimg==""){alert("Url ảnh không được để trống");$("#tad-nhapurl input").focus();return false}
window.history.pushState("object or string","Title","?img="+urlimg);$("#tad-urlimg").html("<img  src='"+urlimg+"' class='img-thumbnail w-100' id='imgurl' alt='Tạo ảnh đẹp'/>");});$("#chenchutad").click(function(){
var max=500;var tyle=$("#imgurl").height()/$("#imgurl").width();canvas.setDimensions({width:max,height:max*tyle});canvas.setBackgroundImage(urlimg,canvas.renderAll.bind(canvas),{crossOrigin:"Anonymous",width:max,height:max*tyle});b2.show();b02.show();b1.hide();});$("#tad-chenchu").click(function(){$("#inputtext").val("");addText.show();okText.hide();});var addText=$("#addText");var okText=$("#okText").hide();$("#addText").click(function(){var tw=canvas.width/2;var nd=$("#inputtext").val();if(nd.length==0){alert("Vui lòng nhập chữ");$("#inputtext").focus();return false}
